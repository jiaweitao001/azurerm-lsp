...
import (
	...
	"github.com/Azure/azurerm-lsp/internal/azure/types"
...
func convertResourceJson(ctx context.Context, input string, telemetrySender telemetry.Sender) (string, error) {
	...
	telemetrySender.SendEvent(ctx, "ConvertJsonToazurerm", map[string]interface{}{
		"status": "completed",
		"kind":   "resource-json",
		"type":   typeValue,
	})
	...
	importBlock.Body().SetAttributeTraversal("to", hcl.Traversal{hcl.TraverseRoot{Name: "azurerm_resource"}, hcl.TraverseAttr{Name: label}})
...
terraform import azurerm_resource.%s %s?api-version=%s
...
func ParseResourceJson(content string) (*hclwrite.Block, error) {
	...
	block.SetLabels([]string{"azurerm_resource", label})
