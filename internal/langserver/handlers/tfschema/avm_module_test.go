...
func TestAVMModule_ListProperties(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			expected:  []string{"account_credentials", "additional_unattend_contents", "allow_extension_operations", "availability_set_resource_id", "azure_backup_configurations", "boot_diagnostics", "boot_diagnostics_storage_account_uri", "bypass_platform_safety_checks_on_user_schedule_enabled", "capacity_reservation_group_resource_id", "computer_name", "custom_data", "data_disk_managed_disks", "dedicated_host_group_resource_id", "dedicated_host_resource_id", "diagnostic_settings", "disk_controller_type", "edge_zone", "enable_automatic_updates", "enable_telemetry", "encryption_at_host_enabled", "eviction_policy", "extensions", "extensions_time_budget", "gallery_applications", "hotpatching_enabled", "license_type", "location", "lock", "maintenance_configuration_resource_ids", "managed_identities", "max_bid_price", "name", "network_interfaces", "os_disk", "os_type", "patch_assessment_mode", "patch_mode", "plan", "platform_fault_domain", "priority", "provision_vm_agent", "proximity_placement_group_resource_id", "public_ip_configuration_details", "reboot_setting", "resource_group_name", "role_assignments", "role_assignments_system_managed_identity", "run_commands", "run_commands_secrets", "secrets", "secure_boot_enabled", "shutdown_schedules", "sku_size", "source_image_reference", "source_image_resource_id", "tags", "termination_notification", "timeouts", "timezone", "user_data", "virtual_machine_scale_set_resource_id", "vm_additional_capabilities", "vtpm_enabled", "winrm_listeners", "zone"},
		},
		{
